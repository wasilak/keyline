...
v0.2.0 (Observability & Integration) shipped 2026-05-18. No active milestone. See backlog below for next candidate.
...
- ✓ Expanded Prometheus metrics: ES user management, LDAP ops, circuit breaker state (METRICS-01–05) — v0.2.0
- ✓ OTel log bridge (opt-in via otel_enabled) + auth engine spans (OTEL-01–03) — v0.2.0
- ✓ Structured audit log events for all auth decisions with OTel trace correlation (AUDIT-01–02) — v0.2.0
- ✓ Auth paths reference documentation with manual test commands for all 5 paths (DOC-03) — v0.2.0
- ✓ Secan integration architecture design: Option C (hybrid forwardAuth + proxy) recommended (SECAN-01) — v0.2.0
...
- Secan PoC / implementation — design spike only in v0.2.0; implementation deferred until architecture validated
...
- **Observability baseline (post v0.2.0)**: `/metrics` endpoint with 8 existing + 7 new `keyline_` metrics; loggergo + OTel spans across LDAP, cache, ES upsert; structured audit log on every auth decision
- **Docs site**: Docusaurus at `docs/` directory, published to wasilak.github.io/keyline
- **Secan**: Rust+React ES management GUI; Option C hybrid integration architecture designed in v0.2.0 — implementation deferred

## Constraints

- **Compatibility**: No breaking changes to config YAML structure between v1 → v2 for basic OIDC/Basic Auth users
- **Metrics compatibility**: Existing metric names kept as-is; `keyline_` prefix only on new v0.2.0 metrics
...
| keyline_ prefix for new v0.2.0 metrics only | Avoid breaking existing Grafana dashboards on upgrade | ✓ Good |
| Secan as design spike only in v0.2.0 | "Figure out how and if" — validate architecture before implementation | ✓ Good — Option C documented, impl deferred |
...
---
*Last updated: 2026-05-18 — Milestone v0.2.0 complete*
